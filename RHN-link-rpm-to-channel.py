#!/usr/bin/env python
#
# Script to interact with RHN/Satellite API for checking local RPM's and
...
def get_rpm_info(rpm_file):
    ...
    try:
        ...
        hdr = ts.hdrFromFdno(fdno)
    os.close(fdno)
    return { 'name': hdr[rpm.RPMTAG_NAME], 'version' :
  hdr[rpm.RPMTAG_VERSION], 'package_release':
...
for fichero in glob.glob(options.ficher):
  ...
  try:
    ...
    paquete=server.packages.findByNvrea(rhnSession,datos["name"],datos["version"],datos["package_release"],'',datos["arch"])["id"]
  except:
